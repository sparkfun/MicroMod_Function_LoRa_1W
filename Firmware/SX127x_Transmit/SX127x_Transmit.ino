/*
  RadioLib SX1276 Transmit Example using the RFM97C 915MHz
...
int pin_powerEnable = 4;

// SX1276 has the following connections on the SparkFun MicroMod Artemis:
int pin_cs = 41;
int pin_dio0 = 0;
int pin_nrst = 33;
int pin_dio1 = 34;
...
void setup() {
  ...
  Serial.println("LOW");
  digitalWrite(pin_powerEnable, LOW);
  while (1);

  pinMode(pin_powerEnable, OUTPUT);
  Serial.println("HIGH");
  digitalWrite(pin_powerEnable, HIGH);
  while (1)
  {
    Serial.println("LOW");
    digitalWrite(pin_powerEnable, LOW);
